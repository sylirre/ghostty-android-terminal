 ...
 * plain exit() would run the atexit handlers and DSO destructors
 * inherited from the Android runtime; _exit() after an explicit
 * kill_all_tracees() performs exactly the cleanup PRoot needs (see the
 * atexit(kill_all_tracees) in tracee/event.c) and nothing else.  */
...
#ifdef PROOT_JNI
	{
		int last_exit_status = event_loop();
		kill_all_tracees();
