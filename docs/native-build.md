...
arm64chroot (an AArch64 Linux user-space emulator, vendored from a sibling
project into `native/arm64chroot/`) is compiled by the same CMake build — no
extra toolchain. Its `src/main.c` compiles `main()` as `arm64chroot_main()`
(under `ANDROID_JNI`), which `pty_jni.c` calls in the fork()ed PTY child;
it links as a static library into `libterm.so`. The CMake source list mirrors
the `SRCS` in `native/arm64chroot/Makefile`.

Nothing is exec'd: arm64chroot *emulates* the aarch64 guest (an interpreter,
or a translating `--jit`), and a guest `execve` is an in-process ELF reload —
so there is no loader, no `PROOT_*` environment, and none of the W^X
exec-dodging the old PRoot needed (targetSdk ≥ 29 cannot `execve()` under app
data). The same source list builds for both ABIs: the JIT backends self-select
by host `#ifdef` (an arm64 backend on arm64-v8a, an x86-64 backend on x86_64),
and the aarch64 rootfs runs emulated on both.

The `--jit` code cache is W^X-aware (RWX anon → `memfd` dual-map under SELinux
`execmem` → interpreter fallback). The lib links `-latomic` for the wide host
atomics the guest's LSE/exclusive ops lower to (`target_link_libraries(term …
atomic)`), and compiles clear of `libterm.so`'s `-Wall -Werror` as its own
`-w` target.
