...
public final class TerminalNative {
    ...
    /**
     * Opens a PTY and forks a child that enters arm64chroot's main() in-process
     * (libterm.so links the emulator; nothing is exec'd — it is a pure AArch64
     * user-space emulator, so the guest never execs a host binary either). args
     * is the full arm64chroot argv including argv[0].
     */
    public static native int ptyCreateEmulator(String[] args, String[] env,
            String cwd, int cols, int rows, int cellWidthPx, int cellHeightPx,
            int[] pidOut) throws java.io.IOException;
